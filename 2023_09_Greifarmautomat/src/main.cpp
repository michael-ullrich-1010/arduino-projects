...
const int axisX_stepper_maxspeed = 8192;//2048; 4096 // stepsPerRevolution change this to fit the number of steps per revolution
const int axisX_stepper_accel = 2000;         // rolePerMinute Adjustable range of 28BYJ-48 stepper is 0~17 rpm
const int axisX_counter_max = 100;

const int axisY_limit = 10;
const int axisY_stepper_1 = 6;
const int axisY_stepper_2 = 7;
const int axisY_stepper_maxspeed = 4000;//2048; 4096 // stepsPerRevolution change this to fit the number of steps per revolution
const int axisY_stepper_accel = 500;         // rolePerMinute Adjustable range of 28BYJ-48 stepper is 0~17 rpm
const int axisY_stepper_step = 1000;//2048; 4096 // stepsPerRevolution change this to fit the number of steps per revolution
...
void loop()
{
  ...
  if (digitalRead(joyX_plus) == LOW && axisX_counter < axisX_counter_max)
  {
    ...
    axisX_stepper.step(-axisY_stepper_step);
  }
  if (digitalRead(joyX_minus) == LOW && digitalRead(axisX_limit) != LOW)
  {
    ...
    axisX_stepper.step(axisX_stepper_rpm);
  }
  if (digitalRead(joyY_plus) == LOW && axisY_counter < axisY_counter_max)
  {
    ...
    axisY_stepper.step(axisY_stepper_rpm);
  }
